...
//	if (isMobile==false) $('#under-submit-div').css('display','block');
	
	$('#submit-form').submit(function(e) {
		e.preventDefault();
		furl=$('#youtube-url').val();
		tryAnother(furl);
	
		return false;
	});
	
	var myParam = location.search.split('watch=')[1];
	if (typeof myParam !== 'undefined') {
		id = matchYoutubeUrl(myParam);
		if (id!=false) {
			$('#youtube-url').val('https://youtu.be/'+id);
			$('#submit-form').submit();
		}
	}
});
...
function tryAnother(url) {
    
	video = url;
	format = "mp3";

	video = matchYoutubeUrl(url);	
